...
curl -X POST -b cookies  https://$SEVER_IP:$SERVER_PORT/api/datasets -H "Authorization: Basic YWRtaW46YWRtaW4=" -H "Content-Type: multipart/form-data" -F datasetName='indiansDiabetes-logistic-regression-hdfs-dataset' -F version='1.0.0' -F description='indiansDiabetes-logistic-regression-hdfs-dataset' -F sourceType='hdfs' -F destination='file' -F dataFormat='CSV' -F containsHeader='true' -F sourcePath='hdfs://localhost:9000/ml/IndiansDiabetes.csv' -F file='hdfs://localhost:9000/ml/IndiansDiabetes.csv' -k
sleep 10
...
project=$(curl -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" -v https://$SEVER_IP:$SERVER_PORT/api/projects/wso2-ml-logistic-regression-with-hdfs-sample-project -k)
sleep 2
...
analysis=$(curl -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" -v https://$SEVER_IP:$SERVER_PORT/api/projects/${projectId}/analyses/wso2-ml-logistic-regression-with-hdfs -k)
sleep 2
