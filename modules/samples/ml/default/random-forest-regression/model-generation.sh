...
echo "testing Random Forest Regression workflow"
...
path=$(pwd)
curl -X POST -b cookies  https://$SEVER_IP:$SERVER_PORT/api/datasets -H "Authorization: Basic YWRtaW46YWRtaW4=" -H "Content-Type: multipart/form-data" -F datasetName='abalone-random-forest-regression-dataset' -F version='1.0.0' -F description='Abalone Dataset' -F sourceType='file' -F destination='file' -F dataFormat='CSV' -F containsHeader='true' -F file=@'/'$path'/abalone.csv' -k
...
project=$(curl -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" -v https://$SEVER_IP:$SERVER_PORT/api/projects/wso2-ml-random-forest-regression-sample-project -k)
sleep 2
...
analysis=$(curl -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" -v https://$SEVER_IP:$SERVER_PORT/api/projects/${projectId}/analyses/wso2-ml-random-forest-regression-sample-analysis -k)
sleep 2
