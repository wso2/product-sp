...
public class RandomEventGenerator implements EventGenerator {
    ...
    public void init(StreamConfigurationDto streamConfiguration) throws InsufficientAttributesException {
         ...
         /*
        * check whether the stream configuration object can be casted to a RandomSimulationDto object.
        * if possible, continue with initialization of generator
        * else, throw an exception
        * */
        if (RandomSimulationDto.class.isInstance(streamConfiguration)) {
            ...
            * else, throw an exception
            * */
            if (CommonOperations.checkAttributes(randomAttributeList.size(), streamAttributes.size())) {
