...
public class RandomEventGenerator implements EventGenerator {
    ...
    public void init(JSONObject streamConfiguration) {
        ...
        try {
            ...
            log.error("Error occurred when generating a random event : " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("Error occurred when initializing random generator for stream '" +
                    randomGenerationConfig.getStreamName() + "' :  " + e.getMessage(), e);
        }
...
//        do nothing
    ...
    public Event poll() {
        if (log.isDebugEnabled()) {
            log.debug("Poll next event from random generator to stream '" + randomGenerationConfig.getStreamName()
                    + "'");
        }
    ...
    public Event peek() {
        if (log.isDebugEnabled()) {
            log.debug("Peek next event of random generator for stream '" + randomGenerationConfig.getStreamName() +
                    "'.");
        }
        return nextEvent;
    }
    ...
    public void getNextEvent() {
        if (log.isDebugEnabled()) {
            log.debug("get next event of random generator for stream '" + randomGenerationConfig.getStreamName()
                    + "'");
        }
