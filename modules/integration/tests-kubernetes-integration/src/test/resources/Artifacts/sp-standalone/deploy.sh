...
echo $host
echo "Waiting DAS to launch on http://${host}:${sp_port}"
sleep 10
...
echo $(date)" Waiting for server startup!"
  STATUS=$(curl -s -o /dev/null -w '%{http_code}' http://${host}:${sp_port}/siddhi-apps)
...
cat > sp-standalone/test-deployment.json << EOF1
$json
EOF1
